...
* GUI experience: I have use it (using a 25 inch HDMI monitor/ Raspberry 7 inch LCD) as Kodi media center and Volumio music player. 
  
  * Kodi: it can connect to my file server (on another Raspberry Pi), play full HD videos with ease and occationally play some musics. However, if I load movie database like I use Kodi on my PC, the Raspberry Pi became painfully slow. So it is okay, but you cannot have the full experience, just is a fine device to play movies and musics.
